...
class TestBooleanNetwork:
    def test_creation_single_element(self):
        ...
        assert network.adj_matrix[0, 0] == True
        assert network.adj_matrix[0, 1] == False

    def test_symmetry(self):
        edges: List[BooleanEdge] = [(0,1)]
        network = BooleanNetwork(2, edges, directed=False)
        assert network.adj_matrix[0, 1] == True
        assert network.adj_matrix[1, 0] == True
        assert network.adj_matrix[0, 0] == False
        assert network.adj_matrix[1, 1] == False

    def test_directed(self):
        edges: List[BooleanEdge] = [(0,1)]
        network = BooleanNetwork(2, edges, directed=True)
        assert network.adj_matrix[0, 1] == True
        assert network.adj_matrix[1, 0] == False
        assert network.adj_matrix[0, 0] == False
        assert network.adj_matrix[1, 1] == False
    ...
    def test_dot_product_propagation(self):
        ...
        propagation = network.adj_matrix.dot(state)
